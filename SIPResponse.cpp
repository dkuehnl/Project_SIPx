...
void SIPResponse::parse_request_line() {
    auto code_start = m_request_line.find("SIP/2.0");
    if (code_start == std::string_view::npos) {
        std::cerr << "[SIPRequest::parse_request_line()]: Malformed Request-Line, no SIP/2.0 found!" << std::endl;
        return;
    }
    auto code_sep = m_request_line.find(' ', code_start + 8);
    if (code_sep == std::string_view::npos) {
        std::cerr << "[SIPRequest::parse_request_line()]: Malformed Request-Line!" << std::endl;
        return;
    }

    std::string_view temp_code = m_request_line.substr(code_start + 8, code_sep - (code_start + 8));
    uint16_t temp_code_val = 0;
    try {
        temp_code_val = static_cast<uint16_t>(std::stoi(std::string(temp_code)));
    } catch (const std::exception& e) {
        std::cerr << "[SIPRequest::parse_request_line()]: Invalid response code: " << e.what() << std::endl;
        return;
    }
